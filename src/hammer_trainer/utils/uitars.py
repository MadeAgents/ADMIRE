...
def parsing_response_to_android_world_code(
    parsed_response,
    obs_image_height,
    obs_image_width,
):
    action_type = copy.deepcopy(parsed_response["action_type"])
    action_params = copy.deepcopy(parsed_response["action_inputs"])

    match action_type:
        case "click":
            x1, y1, x2, y2 = ast.literal_eval(action_params["start_box"])
            x = int((x1 + x2) * obs_image_width / 2)
            y = int((y1 + y2) * obs_image_height / 2)
            action = {
                "name": "click",
                "arguments": json.dumps({"x": x, "y": y}, ensure_ascii=False),
            }
        case "long_press":
            x1, y1, x2, y2 = ast.literal_eval(action_params["start_box"])
            x = int((x1 + x2) * obs_image_width / 2)
        ...
        case "scroll":
            ...
            action = {
                ...
                "arguments": json.dumps(
                    {"x": x, "y": y, "direction": direction}, ensure_ascii=False
                ),
            }
        case "open_app":
            ...
            action = {
                "name": "open_app",
                "arguments": json.dumps({"app_name": app_name}, ensure_ascii=False),
            }
        case "drag":
            x1, y1, x2, y2 = ast.literal_eval(action_params["start_box"])
            x = int((x1 + x2) * obs_image_width / 2)
            y = int((y1 + y2) * obs_image_height / 2)
            touch_xy = [x, y]
            x1, y1, x2, y2 = ast.literal_eval(action_params["end_box"])
            x = int((x1 + x2) * obs_image_width / 2)
            y = int((y1 + y2) * obs_image_height / 2)
            lift_xy = [x, y]
            action = {
                "name": "drag_and_drop",
                "arguments": json.dumps(
                    {"touch_xy": touch_xy, "lift_xy": lift_xy}, ensure_ascii=False
                ),
            }
        case "press_home":
            action = {
        ...
        case "finished":
            action = {
                "name": "status",
                "arguments": json.dumps({"goal_status": "complete"}, ensure_ascii=False),
            }
        case "wait":
            action = {
                "name": "wait",
