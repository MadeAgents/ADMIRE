...
def compute_data_metrics(batch: DataProto, use_critic: bool = False) -> Dict[str, Any]:
    ...
    prompt_mask = (
        torch.logical_and(batch.batch["attention_mask"], batch.batch["labels"] == -100)
    ).bool()
    ...
    if advantages.size(-1) != response_mask.size(-1):
        # print(advantages.shape)
        advantages = F.pad(advantages, (0, 1, 0, 0), mode="constant", value=0)
